...
class CloseConsolesCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:syncconsoles')
            ->setDescription('Désactive les consoles actives depuis 2h');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $dateActivationConsole = new \DateTime('-2 hours');
//dump($dateActivationConsole);
        $query = $em->createQuery('SELECT n FROM AppBundle:Instance n WHERE n.etatConsole = 1 and n.dateActivationConsole < :dateActivationConsole')->setParameter('dateActivationConsole',$dateActivationConsole);
        $instances = $query->getResult();
        foreach ($instances as $instance) {
           // dump($instance->getName());
            $instance->setEtatConsole(false);
            $em->persist($instance);
        }
        $em->flush();
       // dump($instances);
